		...
		btnBuscar.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (!edtCodigo.getText().matches("[0-9]+")) {
					mensagem("Buscar", "Somente números");
					return;
				}

				btnBuscar.setEnabled(false);
				long codigo = Long.parseLong(edtCodigo.getText());
				...
				Display.getCurrent().asyncExec(new Runnable() {
					...
					public void run() {
						try {
							...
							if (jogo != null) {
								lbTitulo.setText(jogo.getTitulo());
								lbDataLancamento
										.setText(new SimpleDateFormat("dd/MM/yyyy").format(jogo.getDataLancamento().getTime()));
								lbGenero.setText(jogo.getGenero());
								lbDescricao.setText(jogo.getDescricao());
							} else {
								mensagem("Buscar", "Jogo não encontrado!");
							}
						} catch (AxisFault e1) {
							e1.printStackTrace();
							mensagem("Buscar", e1.getMessage());
						} catch (RemoteException e1) {					
							e1.printStackTrace();
							mensagem("Buscar", e1.getMessage());
						} finally {
